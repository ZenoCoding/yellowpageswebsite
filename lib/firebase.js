...
const mapImageSnapshot = (snapshot) => {
	...
	return {
		...
		sourceTitle: typeof data.sourceTitle === 'string' ? data.sourceTitle : null,
...
export async function getArticleContent(id) {
	...
	if (!article.exists()) {
		const error = new Error(`Article ${id} does not exist.`);
		error.code = 'article/not-found';
		throw error;
	}
	const cachedMarkdown = typeof cont.markdown === 'string' ? cont.markdown : null;
	if ((typeof cont.path !== 'string' || cont.path.trim().length === 0) && cachedMarkdown === null) {
		const error = new Error(`Article ${id} is missing a storage path.`);
		error.code = 'article/missing-storage-path';
		throw error;
	}

	let rawMarkdown = cachedMarkdown;
	if (cachedMarkdown === null && typeof cont.path === 'string' && cont.path.trim().length > 0) {
		const markdownRef = ref(storage, cont.path);
		const bytes = await getBytes(markdownRef);
		const dec = new TextDecoder();
		rawMarkdown = dec.decode(bytes);
	}
