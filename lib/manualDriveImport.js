...
export const inspectDriveSource = async ({source, accessToken}) => {
    ...
    } else if (root.mimeType === GOOGLE_DOC_MIME_TYPE) {
        files = [{...root, depth: 0, accessError: null}];
    } else {
        throw new Error('The pasted link must point to a Google Drive folder or Google Doc.');
    }
...
export const prepareVisionImages = async ({inspection, accessToken, maximumImages = 6}) => {
    if (typeof window === 'undefined' || !inspection || !accessToken) return {images: [], warnings: []};
    const candidates = [
    ...
    for (const candidate of candidates) {
        try {
            ...
            images.push({
                ...
                dataUrl: await blobToVisionDataUrl(await response.blob()),
            });
        } catch (error) {
