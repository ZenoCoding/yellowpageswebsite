...
const buildFigureHtml = (record, {imageId}) => {
    ...
    if (record.credit) {
        const prefix = record.creditType === 'source' || record.sourceUrl ? 'Source: ' : 'Photo by ';
        const credit = record.sourceUrl
            ? `<a href="${escapeAttribute(record.sourceUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(record.credit)}</a>`
            : escapeHtml(record.credit);
        captionParts.push(`<span class="article-figure__credit">${prefix}${credit}</span>`);
    }
    const captionHtml =
