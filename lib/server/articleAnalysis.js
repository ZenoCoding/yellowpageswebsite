...
export const analyzeArticleSubmission = async ({submission, userId}) => {
    ...
    const visionImages = Array.isArray(submission?.visionImages)
        ...
        : [];
    if (preparedSource.documents.length === 0) {
        const error = new Error('This submission has no readable Google Docs to analyze.');
        error.status = 400;
        throw error;
    }
...
Choose the single document tab most likely to contain the publishable article. Distinguish finished prose from pitches, outlines, interviews, transcripts, captions, research notes, and competing drafts. An empty tab named Final Draft is not evidence of readiness. If multiple plausible drafts exist, choose the best candidate but set readiness to needs_review and explain the ambiguity.
